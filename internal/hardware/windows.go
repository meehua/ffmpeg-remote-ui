...
//
// HwNode 也留空，尽管每个子键都带一个序号。理由是这两种编号对不上：注册表列的是
// 「显示类驱动」，FFmpeg 的 d3d11va 数的是 DXGI 的物理适配器，两套枚举各有各的成员，
// 谁也不保证第几个子键就是 FFmpeg 的第几号。把子键序号当设备值是在替用户猜，而猜错
// 的表现就是「打开编码器失败」。「哪个值能用」因此只剩一个来源：让 FFmpeg 真的初始化
// 一次（ffmpeg.ProbeHWDevice）。
//
