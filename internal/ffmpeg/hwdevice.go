...
// 按类型各不相同（d3d11va 会说它用的是哪块设备，cuda 在 verbose、debug、trace 三级
// 日志下都只说它加载了 cuDeviceGetName 这类符号，从不提设备名），从它的输出里抠设备名
// 只会抠出一层脆壳，而且 qsv 报的还是它自己挑中的子设备，与用户填的那个值不是一回事。
// 设备列表因此由服务器自己的检测给出，实测只负责说哪个值能起来。
//
